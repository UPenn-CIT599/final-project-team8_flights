...
public class AirportReader {
   ...
   *00 airportID - String //OpenFlights database unique ID
   *01 airportName - String
   *02 cityName - String
   *03 countryName - String
   *04 IATACode - String
   *05 ICAOCode- String
   *06 latitude - Double
   *07 longtitude - Double
   *08 altitude - Integer // in feet
   *09 timezone - Double // hours offset from UTC
   *10 DST - String //daylight saving time
   *11 tzTimezone - String //eg America/Los_Angeles
   *12 type - String //airport;station;port;unknown
   *13 source - String //source of this data
   */
  ...
  public AirportReader(String fileName) {
    ...
    try {
      ...
      while (scan.hasNextLine()) {
        ...
        Airport airport =
            new Airport(airportID, airportName, cityName, countryName, IATACode);
  ...
  /**
   * checks if String of integer is empty before running Integer.parseInt class; return -1 if empty,
   * otherwise return integer by running Integer.parseInt
   * 
